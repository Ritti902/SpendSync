...
export function IncomeRow({
  ...
  const category = getCategory(income.category || "other", categories, "income");
  const date = formatDisplayDate(income.date);
  ...
  return (
    <div className="group flex items-start gap-3 rounded-2xl border border-border/50 bg-card/40 p-3 transition-all hover:border-primary/40 hover:bg-card/70">
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-lg sm:h-11 sm:w-11 sm:text-xl" style={categoryStyle(category)}>
        {category.emoji}
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex min-w-0 flex-col gap-1 sm:flex-row sm:items-center sm:justify-between sm:gap-2">
          <p className="truncate font-medium">{income.source || category.label}</p>
          <p className="break-words font-display text-sm font-semibold tabular-nums text-success sm:text-base">
        ...
        {income.note && <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{income.note}</p>}
      </div>
